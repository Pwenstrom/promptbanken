...
- [~] Premium-mallar/-arbetsflöden + MCP till premium + export av premium — **innehåll klart, UI/MCP kvar.**
  ...
  - [ ] **Fas 1b (UI):** ny Pro-flik/sektion i `promptbanken.html`/`script.js` som anropar `list_pro_templates()`, visar de 7 områdena, teaser-lås (hänglås + "Uppgradera till Pro"-CTA kopplad till invite-flödet) på låsta kort, upplåst copy/anpassa-flöde för Pro. Beslut: teaser synligt för alla.
  - [ ] **Fas 2 (MCP):** exponera premium-mallar för Pro-workspaces via MCP (plan-medveten gren, likt `get_workspace_prompts_for_key`).
