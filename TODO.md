...
6. [ ] Ny statisk sida `invite.html` + JS: läs `?token=`, kräv inloggning (skicka till login och behåll länken om ej inloggad), anropa RPC, visa resultat.
7. [ ] Manuell process för MVP (ingen adminpanel): skapa invites direkt i Supabase SQL Editor, kopiera token, skicka länk.
