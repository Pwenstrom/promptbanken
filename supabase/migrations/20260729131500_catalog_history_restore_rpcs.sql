...
-- Restore RPCs for packages (Task 3): admin_list_package_history and
...
-- Note: a function defined with `returns table(...)` does not create a
-- reusable named composite type (unlike a function with genuine OUT
-- parameters) -- `returns setof app_private.list_package_history` would fail
-- with "type does not exist". The wrapper below restates the same column
-- list explicitly instead of referencing the app_private function as a type.
...
revoke all on function app_private.list_package_history(uuid) from public;
grant execute on function app_private.list_package_history(uuid) to authenticated;

revoke all on function public.admin_list_package_history(uuid) from public;
grant execute on function public.admin_list_package_history(uuid) to authenticated;
...
begin
    ...
    if v_history.table_name = 'catalog_package_items' then
        ...
        insert into public.catalog_packages (id, slug, package_type, status, icon_key, image_key, color_theme, updated_by)
        select
            (v_history.row_data->>'id')::uuid,
            v_history.row_data->>'slug',
...
revoke all on function app_private.restore_package_version(bigint, boolean) from public;
grant execute on function app_private.restore_package_version(bigint, boolean) to authenticated;

revoke all on function public.admin_restore_package_version(bigint, boolean) from public;
grant execute on function public.admin_restore_package_version(bigint, boolean) to authenticated;
