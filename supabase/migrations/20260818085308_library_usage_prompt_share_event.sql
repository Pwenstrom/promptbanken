-- 20260818090000_library_usage_prompt_share_event.sql
-- Lägger till 'prompt_share' som tillåtet event_type för
-- track_library_usage_event, samma mönster som 'package_share'
-- (20260812090000_library_usage_package_share_event.sql).
--
-- Delningsknappen i katalogens promptdetaljpanel (tidigare bara synlig för
-- paket) visas nu även för enskilda prompter och behöver en egen
-- händelsetyp för att inte blandas med 'prompt_view'.
...
alter table public.library_usage_events
    add constraint library_usage_event_type_check
      check (event_type in (
        'prompt_view', 'prompt_copy', 'prompt_get', 'prompt_list', 'prompt_share',
