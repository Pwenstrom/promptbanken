...
**Status (2026-07-17):** Helper functions and all 30+4 call-site conversions were already implemented on `main` before this branch started (commit `3c173fb`). This branch only tightened `RAW_DB_ERROR_PATTERNS` to match this task's exact spec (removed 3 extra patterns), then — during Task 5's manual testing — found and fixed a real gap: an unquoted `"... does not exist"` Postgres error format wasn't caught by any pattern, and was leaking to users. Added a `'does not exist'` pattern (commit `7fe19d1`), verified via live Playwright test against staging.
...
- [x] **Step 1: Add the detection + wrapper functions**
...
- [x] **Step 2: Verify the helper in isolation**
...
- [x] **Step 3: Replace every `setStatus(error.message || '...', true)` call site**
...
- [x] **Step 4: Handle the two special-cased call sites separately**
...
- [x] **Step 5: Handle the two `setUpgradeStatus` call sites**
...
- [x] **Step 6: Verify no `error.message ||` pattern remains**
...
**Status (2026-07-17):** Already implemented on `main` before this branch started. Verified `renderPersonalOnboarding()` and the `#kom-igang-personlig` HTML section match this task's spec exactly — no diff needed. Confirmed rendering live via Playwright (logged in as a fresh Free test account).
...
- [x] **Step 2: Add the render function**
...
- [x] **Step 3: Wire it into the render lifecycle**
...
- [x] **Step 4: Verify with the temp-disable technique**
...
**Status (2026-07-17):** Already implemented on `main` before this branch started. `testMcpConnection()` deliberately uses `X-MCP-Key` header + `tools/call list_my_prompts` instead of this task's literal `Authorization: Bearer` + `initialize` spec — that's correct, not a bug: it matches the real hosted-MCP protocol (`X-MCP-Key` per header, per CLAUDE.md), which wasn't finalized when this task text was written. Left as-is. Live click-through with a real key still needs a human (per this task's own Step 4 note) — blocked here only by the Free-plan 1-key limit already being in use by the test account, not by any code issue.
...
- [x] **Step 1: Check CORS from the browser console first**
...
- [x] **Step 2: Add the button to the reveal panel**
...
- [x] **Step 3: Add the test-connection function**
...
**Status (2026-07-17):** Already implemented on `main` before this branch started — the exact paragraph from this task's Step 2 is present verbatim in `admin.html`'s `#installningar` section.
...
**Status (2026-07-17):** Done via Playwright at 375px against a live dev server (real login as a Free test account, not just the temp-disable technique). `login.html` and `admin.html` Översikt: no page-level horizontal overflow (`scrollWidth === clientWidth` on both), MCP-key table scrolls contained within its own `overflow-x:auto` box (intentional, by design), `secret-reveal-row code` still has `overflow-wrap:anywhere`. Found and fixed a real bug in the process (see Task 1's status note: the `review_note` column was missing from staging, causing a raw-error leak) — filed and fixed as part of Task 1's scope since that's the mechanism it broke, not as a new Task 5 deliverable.
...
- [x] **Step 1: Simulate a 375px viewport via iframe**
...
- [x] **Step 2: Check `login.html` at 375px**
...
- [x] **Step 3: Check `admin.html`'s Översikt tab at 375px, logged in**
...
**Status (2026-07-17):** Done against the shared staging Supabase project via the Supabase MCP (`set role anon`/`authenticated` + `request.jwt.claim.sub` simulation for genuine RLS enforcement, not just service-role checks). All 7 files pass: `rls_test_plan.sql` (6/6 blocks — anon/draft/archived visibility, workspace isolation with both positive and negative controls, profiles role-scoped visibility, api_keys never exposed via views), `verify_a_mcp_boundary.sql` (5/5), `verify_b_join_rejections.sql` (2/2), `addon_no_own_keys.sql`, `addon_member_limit.sql`, `addon_prompt_limit.sql` (2/2), `context_mcp_scope.sql` (5/5) — the latter three were comment-only "manual scenario" placeholders in the file but were run for real using existing seeded fixtures. All test rows/keys/temporary limit changes were cleaned up and verified restored (0 leftover rows in every case). No regressions found. Full detail in `.superpowers/sdd/progress.md` (branch-local, not committed).
...
- [x] **Step 1: Read each file's setup instructions**
...
- [x] **Step 2: Create the fixtures in a staging/test Supabase project**
...
- [x] **Step 3: Run each block, recording actual vs. expected**
...
- [x] **Step 4: For any failure, open a fix as its own task**
